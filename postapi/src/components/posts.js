...
const PostPage = () =>{
    ...
    useEffect(()=>{
        ...
        const CheckToken = async () =>{
            ...
            const check = await axios.post('http://localhost:5000/db//CheckToken',form_data)
            if(!check.data){
                navigate("/Login")
                ...
                const check = await axios.get('http://localhost:5000/db//GetPosts')
    ...
    const handleSubmit = async  () =>{
        ...
        try{
            ...
            let res = await axios.post('http://localhost:5000/db/MakePost',form_data)
        }
        catch(err){
            console.log(err)
